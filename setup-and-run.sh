...
echo ""
echo -e "${RED}⚠️  IMPORTANT: Activate the conda environment first!${NC}"
echo -e "${YELLOW}   conda activate $CONDA_ENV_NAME${NC}"
echo ""
echo -e "${YELLOW}1. Enable itinerary search on the dates you care about:${NC}"
echo "   # Both steps are per-date and NOT run above: UTC offsets are"
echo "   # DST-dependent, and CONNECTS_TO for all 365 days would be ~228M edges."
echo "   conda activate $CONDA_ENV_NAME"
echo "   python load_bts_data.py --solve-offsets 2025-07-18    # must come first"
echo "   python load_bts_data.py --build-connections 2025-07-18"
echo ""
echo -e "${YELLOW}2. Start Load Testing:${NC}"
echo "   conda activate $CONDA_ENV_NAME"
echo "   locust -f neo4j_flight_load_test.py"
echo "   # Then visit: http://localhost:8089"
echo ""
echo -e "${YELLOW}3. Run Sample Queries:${NC}"
echo "   # Neo4j Browser: http://localhost:7474 (if local)"
echo "   # Or use your Neo4j Aura/hosted instance interface"
echo ""
echo -e "${YELLOW}4. Explore the Data:${NC}"
echo "   conda activate $CONDA_ENV_NAME"
echo "   python -c \"from neo4j import GraphDatabase; # Use your .env credentials\""
echo ""
echo -e "${YELLOW}5. Run Integration Tests (needs step 1 first):${NC}"
echo "   conda activate $CONDA_ENV_NAME"
