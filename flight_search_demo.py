...
class FlightSearchDemo:
    ...
    def search_direct_flights(self, origin, destination, date, hour_range=(8, 12)):
        ...
        if records:
            ...
            for i, flight in enumerate(records, 1):
                ...
                score = flight["score"]
                print(
                    f"   {i}. {dep_time}-{arr_time} ({flight['airline']}) {flight['flight']}"
                )
    ...
    def search_connections(self, origin, destination, date, hour_range=(6, 14)):
        ...
        if records:
            ...
            for i, conn in enumerate(records, 1):
                ...
                print(
                    f"   {i}. {dep_time}-{hub_arr} → {hub_dep}-{arr_time} via {conn['hub']}"
                )
                print(
                    f"      Airlines: {conn['airlines']}, Connection: {int(conn['connection_minutes'])}min"
                )
                print(
                    f"      Total time: {int(conn['total_travel_minutes'])}min, Score: {int(conn['score'])} (incl. 60min penalty)"
                )
        else:
    ...
    def run_search_scenario(self, name, origin_city, dest_city, date, time_range):
        ...
        print("=" * 60)
        print(
            f"Traveler: '{origin_city} to {dest_city} on {date}, departing {time_range[0]:02d}:00-{time_range[1]:02d}:00'"
        )

        # Search direct flights
        direct_count, direct_time = self.search_direct_flights(
            origin, destination, date, time_range
        )

        # Search connections
        connection_count, connection_time = self.search_connections(
            origin, destination, date, time_range
        )
        ...
        print(
            f"✅ Options found: {direct_count} direct + {connection_count} connections"
        )
