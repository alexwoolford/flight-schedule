...
- **A live-looking credential is committed in three places and in git history.** `.env.backup` (tracked; `NEO4J_URI` is an RFC-1918 host) and `.env.example` share a byte-identical `NEO4J_PASSWORD`, and `AGENTS.md:446,449,457` quotes that same password and IP as examples of what never to commit — so the security rulebook leaks the secret it exists to prevent. The value appears in commits 81edc69, a28b421, and aaa4fc8, so untracking the file does not remove it. Correct order: rotate the credential first, then rewrite history. Don't copy these values into code.
