...
- **A credential was committed. Treat it as disclosed — it is still the live password.** This entry replaces an earlier one that said the value "remains fetchable"; that was true when written and is now only half true, so the status is broken out rather than reworded.

  Fixed in `fbfac4a`: `.env.backup` is gone, `.env.example` holds `changeme`, and `AGENTS.md` no longer quotes the value. The three commits that carried it (81edc69, a28b421, aaa4fc8) are **not** ancestors of `HEAD` — verified, `.env.backup` appears in no reachable commit.

  | | status |
  |---|---|
  | reachable history | clean since `fbfac4a` |
  | local unreachable objects | **purged** — `reflog expire --expire-unreachable=now --all` then `gc --prune=now`; all three SHAs now give `fatal: Not a valid object name`, `fsck` reports 0 unreachable commits |
  | hosted copy on GitHub | **still live** — all three still return `.env.backup` (136 bytes) by SHA; request drafted for Support, not yet sent |
  | the password itself | **not rotated**, by the owner's explicit decision |

  Two things to get right if you touch this. **The local purge is not the fix** — `gc --prune=now` alone would not have dropped these, because they were *packed*; the reflog expiry is the load-bearing half. And it reaches only this clone: GitHub serves unreachable objects by SHA indefinitely, so no client-side rewrite closes the public exposure. Only a Support purge does, and it is viable here solely because nothing server-side re-anchors the SHAs (0 forks, `network_count` 0, only `refs/heads/main` and `refs/pull/1/head`).

  **Rotation was declined, so the value is compromised, not merely exposed.** Digest comparison (never the plaintext) confirms `NEO4J_PASSWORD` in the current `.env` is byte-identical to the one in `.env.backup@81edc69` — only the host changed. It has been fetchable from a public repository since 2025-07-31, so anyone who already pulled it holds a working credential and the purge cannot take that back. Never reuse the value, and don't copy it or the RFC-1918 host into code.

  The recurrence mechanism is closed: `.gitignore` covers `.env.*` with `!.env.example`, and the `no-env-files` `pre-commit` hook rejects any staged `.env*` except `.env.example`. That guard matters because `.gitignore` did **not** stop the original commit — `git add -f` overrides it, and the hook does not care about `-f`.
