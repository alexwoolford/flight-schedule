...
class TestPerformanceBaseline:
    ...
    def test_database_has_data(self, neo4j_driver, neo4j_database):
        ...
        with neo4j_driver.session(database=neo4j_database) as session:
            ...
            assert schedule_count >= 1000, f"Need at least 1k schedules for performance testing, got {schedule_count}"
            assert airport_count >= 10, f"Need at least 10 airports, got {airport_count}"
    ...
    def test_schema_validation(self, neo4j_driver, neo4j_database):
        ...
        with neo4j_driver.session(database=neo4j_database) as session:
            ...
            print("✅ Schema validation passed - all required temporal properties present")
    ...
    def test_direct_flight_baseline_performance(self, neo4j_driver, neo4j_database):
        ...
        with neo4j_driver.session(database=neo4j_database) as session:
            ...
            assert query_time < 5000, f"Direct flight query taking too long: {query_time:.1f}ms"
    ...
    def test_connection_flight_baseline_performance(self, neo4j_driver, neo4j_database):
        ...
        with neo4j_driver.session(database=neo4j_database) as session:
            ...
            assert query_time < 15000, f"Connection query taking too long: {query_time:.1f}ms"
            
            if connection_count > 0:
                assert 45 <= avg_connection_time <= 300, f"Average connection time out of range: {avg_connection_time}"
    ...
    def test_temporal_query_baseline_performance(self, neo4j_driver, neo4j_database):
        ...
        with neo4j_driver.session(database=neo4j_database) as session:
            ...
            assert query_time < 10000, f"Temporal query taking too long: {query_time:.1f}ms"
    ...
    def test_multi_hop_routing_baseline(self, neo4j_driver, neo4j_database):
        ...
        with neo4j_driver.session(database=neo4j_database) as session:
            ...
            result = session.run("""
                ...
                LIMIT 10
            """, origin=top_hubs[0], destination=top_hubs[1])
            ...
            assert query_time < 20000, f"Multi-hop query taking too long: {query_time:.1f}ms"
    ...
    def test_baseline_report_summary(self, neo4j_driver, neo4j_database):
        ...
        if hasattr(self, 'direct_flight_baseline'):
            ...
            print(f"✈️  Direct Flights: {df['query_time_ms']:.1f}ms, {df['flight_count']} flights from {df['test_airport']}")
        
        if hasattr(self, 'connection_baseline'):
            cf = self.connection_baseline
            print(f"🔄 Connections: {cf['query_time_ms']:.1f}ms, {cf['connection_count']} valid connections")
        
        if hasattr(self, 'temporal_baseline'):
            tf = self.temporal_baseline
            print(f"🕐 Temporal Filter: {tf['query_time_ms']:.1f}ms, {tf['morning_flights']} morning flights")
        
        if hasattr(self, 'multihop_baseline'):
            mf = self.multihop_baseline
            print(f"🛣️  Multi-hop: {mf['query_time_ms']:.1f}ms, {mf['path_count']} paths for {mf['test_route']}")
...
class TestQueryCorrectness:
    ...
    def test_connection_temporal_logic(self, neo4j_driver, neo4j_database):
        ...
        with neo4j_driver.session(database=neo4j_database) as session:
            ...
            assert min_conn >= 45, f"Found connection shorter than 45 minutes: {min_conn}"
            assert max_conn <= 300, f"Found connection longer than 300 minutes: {max_conn}"
            assert 45 <= avg_conn <= 300, f"Average connection time out of range: {avg_conn}"
    ...
    def test_flight_data_consistency(self, neo4j_driver, neo4j_database):
        ...
        with neo4j_driver.session(database=neo4j_database) as session:
            ...
            assert same_airport == 0, f"Found {same_airport} flights with same origin/destination"
            assert impossible == 0, f"Found {impossible} flights with impossible negative/zero duration"
            assert min_dur > 0, f"Minimum flight duration should be positive, got {min_dur}"
            assert max_dur < 24*60, f"Maximum flight duration should be under 24 hours, got {max_dur}"